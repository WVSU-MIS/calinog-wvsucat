...
def app():
    ...
    st.image(img)
    st.title("WVSU Calinog Campus WVSUCAT Dashboard")      
    st.subheader("(c) 2023 WVSU Management Information System Office")
                 
    st.write("This dashboard is managed by: Dr. Rosario Clarabel C. Contreras, Campus Administrator\ncalinog@wvsu.edu.ph")
    ...
    st.subheader("2023 WVSUCAT Examination Results")
    year = '2023'
    ...
    df_colleges = pd.read_csv('calinog-courses.csv', header=0, sep = ",", encoding='latin')
    ...
    passing_score = st.slider('Passing Score', 30, 100, 60, 5)
